...
export class ProfessionnelComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.professionnelService
      ...
      .subscribe({
        next: (res: HttpResponse<IProfessionnel[]>) => {
  ...
  trackId(index: number, item: IProfessionnel): number {
    return item.id!;
  }

  delete(professionnel: IProfessionnel): void {
    const modalRef = this.modalService.open(ProfessionnelDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IProfessionnel[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
